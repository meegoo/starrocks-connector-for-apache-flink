...
`testEndToEndMultiPartition`、`testCheckpointTriggeredFlushDataIntegrity` 等若在主线程直接 `env.execute()`，作业卡死会导致 Surefire 长时间不打印 `[INFO] Results:`。当前实现通过 `runFlinkJobWithTimeout`（默认 **30s**）在子线程执行 `execute` 并在超时后失败断言，便于与 `timeout` 命令配合定位。
